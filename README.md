...
| `theme`                       | Appearance defaults, each applying only until the user picks something in the settings drawer: `primaryColor`, `headerColor` (`"primary"` or a CSS color), `dark` (`true` / `false` / `"auto"` = follow the OS), `compact`, `skin` (`"default"` / `"brutalist"` / `"liquid-glass"`), `menuMode` (`"normal"` sidebar / `"split"` categories in the header / `"dual"` two-column sidebar / `"top"` whole menu in the header / `"group"` categories as flat group titles / `"top-split"` categories in the header with their children in a second row). The older top-level `darkTheme`, `compactTheme`, `brutalistTheme` and `liquidGlass` keys still work. |
