...
import {
  eruptModelConvertViewKeys, eruptValueToObject, viewToAlainTableConfig
...
export class TableComponent implements OnInit {
  ...
  ngOnInit() {
    this.dataService.getEruptBuild(this.eruptName).subscribe(
      em => {
        this.eruptModel = em;
        this.eruptModel.views = eruptModelConvertViewKeys(this.eruptModel);
        ...
        em.eruptFieldModels.forEach((field, i) => {
          ...
          if (field.eruptFieldJson.edit.type === EditType.TAB) {
            ...
            this.dataService.getEruptBuild(field.fieldReturnName).subscribe(
              subEm => {
                subEm.views = eruptModelConvertViewKeys(subEm);
  ...
  buildTableConfig() {
    ...
    this.columns.push(...viewToAlainTableConfig(this.eruptModel.views));
    const operators = [];
