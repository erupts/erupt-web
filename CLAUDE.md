...
- Three lazy theme bundles (angular.json `inject:false`, stable non-hashed names): `styles.dark.less` → `style.dark.css`, `styles.compact.less` → `style.compact.css`, and `styles.compact-dark.less` (dark + compact merged) → `style.compact-dark.css`. `index.html` owns the single `<link id="site-theme-style">` swapped between them via `window.eruptApplyDarkTheme(bool)` / `window.eruptApplyCompactTheme(bool)` (applied pre-bootstrap to avoid a flash; `dark-theme` localStorage supports `"auto"` = follow the OS scheme, with a live `matchMedia` listener). **Changing angular.json bundles requires a dev-server restart** — a stale server answers the css URLs with the SPA-fallback HTML (empty stylesheet, no error).
